...
export class RestaurantRepository {
  ...
  async updateRestaurant(id: string, data: any) {
    ...
    params.push(id);
    const query = `UPDATE restaurants SET ${updates.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = $${paramIndex} RETURNING *`;
  ...
  async softDeleteRestaurant(id: string) {
    const result = await AppDataSource.query(
      `UPDATE restaurants SET status = 'DELETED', deleted_at = CURRENT_TIMESTAMP WHERE id = $1 RETURNING id`,
      [id]
    );
    return result[0] || null;
  }
  ...
  /**
   * Get for management (includes deleted)
   */
  async findByIdForManagement(id: string) {
    const result = await AppDataSource.query(`SELECT * FROM restaurants WHERE id = $1`, [id]);
    return result[0] || null;
  }
}