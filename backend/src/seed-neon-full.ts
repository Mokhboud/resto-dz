...
const seedNeon = async () => {
  try {
    ...
    for (const c of categories) {
      await neonDataSource.query(
        `INSERT INTO categories (name_fr, name_ar, name_en, icon) VALUES ($1, $2, $3, $4) ON CONFLICT (name_fr) DO NOTHING`,
        [c.name_fr, c.name_ar, c.name_en, c.icon]
      );
    }
    console.log('✅ 17 categories seeded');
    ...
    for (const c of cuisines) {
      await neonDataSource.query(
        `INSERT INTO cuisines (name_fr, name_ar, name_en, icon) VALUES ($1, $2, $3, $4) ON CONFLICT (name_fr) DO NOTHING`,
        [c.name_fr, c.name_ar, c.name_en, c.icon]
      );
    }
    ...
    const restaurants = [
      { name: 'Dar El Bahdja', desc: 'Restaurant traditionnel algérien', phone: '021234567', addr: '12 Rue Didouche Mourad, Alger', wilaya_id: 16, lat: 36.7650, lng: 3.0510, price: 2, verified: true },
      { name: 'Pizza Roma', desc: 'Pizzas artisanales', phone: '021345678', addr: '45 Boulevard Krim Belkacem, Alger', wilaya_id: 16, lat: 36.7710, lng: 3.0550, price: 2, verified: true },
      { name: 'Le Petit Resto', desc: 'Cuisine méditerranéenne', phone: '041234567', addr: 'Boulevard de la Soummam, Oran', wilaya_id: 31, lat: 35.7010, lng: -0.6380, price: 2, verified: true },
      { name: 'Restaurant Cirta', desc: 'Cuisine constantinoise', phone: '031234567', addr: 'Rue Belouizdad, Constantine', wilaya_id: 25, lat: 36.3650, lng: 6.6140, price: 2, verified: true },
    ];
    for (const r of restaurants) {
      const slug = r.name.toLowerCase().replace(/[^a-z0-9\s-]/g, '').replace(/\s+/g, '-');
      
      // Insert restaurant WITHOUT location first
      const result = await neonDataSource.query(
        `INSERT INTO restaurants (name, slug, description, phone, address, wilaya_id, latitude, longitude, price_level, status, verified)
         ...
         RETURNING id`,
        [r.name, slug, r.desc, r.phone, r.addr, r.wilaya_id, r.lat, r.lng, r.price, r.verified]
      );
